...
-(OEPocketsphinxController *) pocket_sphinx_controller{
    if (pocket_sphinx_controller == nil){
        ...
        // pocket_sphinx_controller.returnNbest = TRUE;
        // pocket_sphinx_controller.nBestNumber = 5;
    }
...
// OneOfTheseCanBeSaidWithOptionalRepetitions // This indicates that exactly one selection from the words in the array can be said, one time or more, but that all of the words can also be omitted from the utterance.
    ...
    //dictionary attempt
    NSError *error = [self.language_model_generator generateGrammarFromDictionary:languageDict withFilesNamed:languageName forAcousticModelAtPath:self.acoustic_model];
  ...
  //       NSString *lmFile = [dynamicLanguageGenerationResultsDictionary objectForKey:@"LMFile"];
        ...
        switch ([self.started_listening intValue]) {
            ...
            case 1:
                ...
                // [self.pocket_sphinx_controller setVadThreshold:3.5];
                ...
                [self.pocket_sphinx_controller setVadThreshold:3.5]; // change the threshold for speech detection
                break;
                // Otherwise, start listening with language model
            default:
                NSLog(@"***Starting with new lm and JSGF");              
                [self.pocket_sphinx_controller setVadThreshold:3.5];  // change the threshold for speech detection
                [self.pocket_sphinx_controller startListeningWithLanguageModelAtPath:self.path_to_dynamic_language_model dictionaryAtPath:self.path_to_dynamic_grammar acousticModelAtPath:self.acoustic_model languageModelIsJSGF:TRUE];
                break;
        }
    }
}
...
-(void)startListeningWithLanguageModelAtPath:(CDVInvokedUrlCommand*)command{
    ...
    [self.pocket_sphinx_controller startListeningWithLanguageModelAtPath:self.current_language_model dictionaryAtPath:self.current_dictionary acousticModelAtPath:self.acoustic_model languageModelIsJSGF:TRUE];
}
